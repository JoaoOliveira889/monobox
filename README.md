...
- **Focused dashboard** — Neovim-like dense aesthetic, transparent selected rows with bold cyan pointer (`▶`), compact 2-line header, stable-width service icons, and contextual quick actions
- **Animated Startup Screen** — smooth brand wordmark, scanning indicator, and progress animation matching MonoGit
- **Precision mouse wheel scrolling** — strict 1-item / 1-line precision scroll with release filtering and debounce
...
- **User Config (`~/.config/monobox/config.yaml`)** — atomic writes with restrictive permissions (`0600` file, `0700` dir)
...
- **Categorized Shortcuts (`?` / `Ctrl+P`)** — MonoGit-style multi-column shortcuts overlay with clean dividers and rounded borders
- **Security first** — discrete exec arguments, validated port numbers (1-65535), no shell injection, log file and config at `0600`
...
| Key | Action |
|-----|--------|
| `↑` / `k`, `↓` / `j` | Navigate containers / scroll modal (1 item/line per notch) |
...
| `Ctrl+D` | Half page down (5 items) |
| `Ctrl+U` | Half page up (5 items) |
